...
class DialogueCharacterEditorState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput && !animationDropDown.dropPanel.visible)
		{
			...
			if (FlxG.keys.justPressed.SPACE #if mobile || virtualPad.buttonA.justPressed #end && UI_mainbox.selected_tab_id == 'Character')
			{
			...
			var offsetAdd:Int = 1;
			var speed:Float = 300;
			if (FlxG.keys.pressed.SHIFT #if mobile || virtualPad.buttonB.justPressed #end)
			{
				speed = 1200;
				offsetAdd = 10;
			}
				...
				&& character.dialogueAnimations.exists(curSelectedAnim))
			{
				...
				var controlArrayLoop:Array<Bool> = [
					FlxG.keys.justPressed.A #if mobile || virtualPad.buttonLeft.justPressed #end,
					FlxG.keys.justPressed.W #if mobile || virtualPad.buttonUp.justPressed #end,
					FlxG.keys.justPressed.D #if mobile || virtualPad.buttonRight.justPressed #end,
					FlxG.keys.justPressed.S #if mobile || virtualPad.buttonDown.justPressed #end];
			...
			if (FlxG.keys.justPressed.H #if mobile || virtualPad.buttonX.justPressed #end)
			{
				if (UI_mainbox.selected_tab_id == 'Animations')
				{
					currentGhosts++;
			...
			if (FlxG.keys.justPressed.R #if mobile || virtualPad.buttonY.justPressed #end)
			{
				camGame.zoom = 1;
	...
	function saveCharacter()
	{
		...
		if (data.length > 0)
		{
			...
			#if mobile
			MobileUtil.save(characterName, ".json", data);
			#else
			_file = new FileReference();
