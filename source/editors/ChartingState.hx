...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			...
			if (FlxG.keys.pressed.W || FlxG.keys.pressed.S #if mobile || virtualPad.buttonUp.pressed || virtualPad.buttonDown.pressed #end)
			{
				...
				if (FlxG.keys.pressed.CONTROL)
					holdingShift = 0.25;
				else if (FlxG.keys.pressed.SHIFT #if mobile || virtualPad.buttonY.pressed #end)
					holdingShift = 4;

				var daTime:Float = 700 * FlxG.elapsed * holdingShift;

				if (FlxG.keys.pressed.W #if mobile || virtualPad.buttonUp.pressed #end)
				{
			...
			if (vortex && !blockInput)
			{
				...
				var feces:Float;
				if (FlxG.keys.justPressed.UP || FlxG.keys.justPressed.DOWN #if mobile || virtualPad.buttonUp.justPressed
					|| virtualPad.buttonDown.justPressed #end)
				{
					FlxG.sound.music.pause();
					...
					var snap:Float = quantization / 4;
					var increase:Float = 1 / snap;
					if (FlxG.keys.pressed.UP #if mobile || virtualPad.buttonUp.pressed #end)
					{
						var fuck:Float = CoolUtil.quantize(beat, snap) - increase;
	...
	private function saveLevel()
	{
		...
		{
			#if mobile
			MobileUtil.save(Paths.formatToSongPath(_song.song) + postfix, ".json", data.trim());
			#else
	...
	private function saveEvents()
	{
		...
		{
			#if mobile
			MobileUtil.save("events", ".json", data.trim());
			#else
			_file = new FileReference();
			_file.addEventListener(Event.COMPLETE, onSaveComplete);
			_file.addEventListener(Event.CANCEL, onSaveCancel);
