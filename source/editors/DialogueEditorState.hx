...
class DialogueEditorState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			...
			var controlText:Array<Bool> = [
				FlxG.keys.justPressed.D #if mobile || virtualPad.buttonLeft.justPressed #end,
				FlxG.keys.justPressed.A #if mobile || virtualPad.buttonRight.justPressed #end];
			...
			else if (FlxG.keys.justPressed.P #if mobile || virtualPad.buttonB.justPressed #end)
			{
	...
	function saveDialogue()
	{
		...
		{
			#if mobile
			MobileUtil.save("dialogue", ".json", data);
			#else
			_file = new FileReference();
