...
class CharacterEditorState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		if(!charDropDown.dropPanel.visible) {
			...
			if (FlxG.keys.justPressed.R #if mobile || virtualPad.buttonZ.justPressed #end) {
				FlxG.camera.zoom = 1;
			}
			...
			if(char.animationsArray.length > 0) {
				...
				if (FlxG.keys.justPressed.S #if mobile || virtualPad.buttonD.justPressed #end || FlxG.keys.justPressed.W #if mobile
				...
				var controlArray:Array<Bool> = [					 FlxG.keys.justPressed.LEFT  #if mobile || virtualPad.buttonLeft.justPressed #end,
